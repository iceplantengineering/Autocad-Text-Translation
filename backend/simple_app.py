...
async def health_check():
    return {"status": "healthy", "timestamp": datetime.now(), "mode": "test"}

@app.get("/")
async def root():
    return {
        "message": "AutoCAD DWG Translator API (Test Mode)",
        "version": "1.0.0",
        "features": [
            "DWG file upload and processing",
            "Chinese text extraction",
            "Mock translation service",
            "Text replacement in DWG files"
        ],
        "note": "This is a test version with mock translation service"
    }
